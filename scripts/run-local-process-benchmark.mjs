...
import { getProcessSuite, processSuites } from './calibration-fixtures.mjs';

function usage() {
  console.error(
    'Usage: node scripts/run-local-process-benchmark.mjs [suite] [field_template]\n' +
    '       node scripts/run-local-process-benchmark.mjs --list\n' +
...
function buildRow(record) {
  return {
    ...
    field_filtered: record.field_filtered_detection_total,
...
function fixtureMeetsProcessChecks(record, processChecks) {
  if (!processChecks) return true;
  if ((processChecks.tracksMin ?? 0) > record.tracks) return false;
  if ((processChecks.mappedMin ?? 0) > record.mapped_keyframe_total) return false;
  if ((processChecks.ballMin ?? 0) > record.ball) return false;
  return true;
}
...
async function runLocalFixture(fixture, fieldTemplate) {
  ...
  const outputDir = path.join(repoRoot, 'benchmark-results', 'local-process-benchmark', 'artifacts');
  ...
  await new Promise((resolve, reject) => {
    ...
    child.on('close', (code) => {
      ...
      else reject(new Error(`Local process fixture ${fixture.id} failed with exit code ${exitCode}`));
    });
  });

  if (!fs.existsSync(outputPath)) {
    throw new Error(`Local process fixture ${fixture.id} did not write output at ${outputPath}`);
  }
  ...
  const record = {
    ...
    process_checks: fixture.processChecks ?? null,
    status: summary.calibration?.status ?? 'unknown',
    failure_code: summary.calibration?.failure_code ?? parsed?.failure_code ?? null,
    failure_message: summary.calibration?.failure_message ?? parsed?.error ?? null,
  ...
  console.log(
    `fixture_status=${record.status} coverage=${record.coverage_ratio} tracks=${record.tracks} ball=${record.ball} mapped=${record.mapped_keyframe_total}`
  );
...
async function main() {
  ...
  if (process.argv.includes('--list')) {
    for (const [suiteName, fixtures] of Object.entries(processSuites)) {
      console.log(`\n${suiteName}`);
  ...
  const fixtures = getProcessSuite(suiteName);
  ...
  const outDir = path.join(process.cwd(), 'benchmark-results', 'local-process-benchmark');
  ...
  const failedExpectations = records.filter((record) => {
    ...
    return !fixtureMeetsProcessChecks(record, record.process_checks);
  });
